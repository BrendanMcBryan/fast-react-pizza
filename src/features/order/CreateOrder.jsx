...
function CreateOrder() {
  ...
  const isLoaadingAddress = addressStatus === "loading";
      ...
      <Form method="POST">
        ...
        <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-center">
          ...
          <div className="grow">
            ...
            {formErrors?.phone && (
              <p className="mt-2 rounded-lg bg-red-300 p-2 text-xs text-red-800">
                {formErrors.phone}
              </p>
            )}
          </div>
...
export async function action({ request }) {
  ...
  if (!isValidPhone(order.phone))
    errors.phone =
      "Please provide Valid Phone Number. We may need to contact you. ";

  if (Object.keys(errors).length > 0) return errors;

  const newOrder = await createOrder(order);
  // * do not over use store.dispatch line
